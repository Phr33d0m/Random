 ...
 *
 *  Some changes made by n0kS Phr33d0m
 *  (almost useless changes of course)
 * 
 * 
...
if(isset($_GET['dat']) && $_GET['dat'] == "sauce"){ 
    ...
    die('<html><head><title>Page Source For: '.__FILE__.'</title><style type="text/css">body {margin: 0px;margin-left: 5px;}.num {border-right: 1px solid;color: gray;float: left;font-family: monospace;font-size: 13px;margin-right: 6pt;padding-right: 6pt;text-align: right;}code {white-space: nowrap;}td {vertical-align: top;}</style></head><body><i>Code also available at <a href="https://github.com/Phr33d0m/Random/blob/master/systeminfo.php">github</a>.</i><table><tr><td class="num"  style="border-left:thin; border-color:#000;">'.$lines.'</td><td class="content">'.$content.'</td></tr></table></body></html>'); 
} 
...
function format_bytes($bytes){ 
    ...
    else if ($bytes < 1048576){ return round($bytes / 1024, 2).'KiB'; } 
    else if ($bytes < 1073741824){ return round($bytes / 1048576, 2).'MiB'; } 
    else if ($bytes < 1099511627776){ return round($bytes / 1073741824, 2).'GiB'; } 
...
//#users, uptime, load avgs, system info
$uptime = exec('uptime'); 
...
//#HDD info from all HDDs availabe (not a very good implementation tho)
$harddrive_used = exec("df | awk '{ SUM += $3} END { print SUM }'");
$harddrive_total = exec("df | awk '{ SUM += $2} END { print SUM }'");
$harddrive_out = format_bytes(kb2bytes($harddrive_used)).'<small> / '.format_bytes(kb2bytes($harddrive_total)).' <small>('.calculate_percentage(kb2bytes($harddrive_used), kb2bytes($harddrive_total)).'% Free)</small></small>'; 
...
<html> 
    <head> 
        <title><? echo exec('hostname'); ?> - shell server</title> <!-- Lets hope hostname doesn't return anything unexpected -->  
    ...
    <body> 
	<div id="links">useful <a href="?dat=sauce">sauce</a></div> 
        server time<span><?=date("j-F-Y H:i:s", time());?></span> <!-- Server time is acutally PHP time since PHP wants to bitch --> 
        ...
        bandwidth used (monthly)<small></small><span><?=$vnstat[8].$vnstat[9].$vnstat[10].$vnstat[11];?></small></small></span> <!-- Monthly bandwidth usage --> 
        <small><small>powered by</small> <? echo exec('cat '.$system.' | head -1');?></small>
    </body> 
</html> 
